...
class Process:
    def __init__(self, instructions= None,pag_loc = 0,priority=99,name=None,parent=None):
        self.parent= parent
    ...
    def operate(self,instruction):
        ...
        if self.pag_loc:
            ...
            parent_core.opcodes[instruction[0]](*instruction[1:],pag_loc=self.pag_loc)
        else:
...
class Core:
    ...
    def OP_MOV(self,adr,*args,pag_loc=None):
        if not pag_loc:
            if len(args)==1:
            ...
            if len(args)==1:
                ...
                self.parent.memory[pag_loc][adr] = args[0] #SAVES first arg in adr
            else:
                
                self.parent.memory[pag_loc][adr] = self.opcodes[args[0]](*args[1:],pag_loc=pag_loc)
                
    def OP_ADD(self,x,y,pag_loc=None):
        if not pag_loc:
            return self.parent.memory[x] + self.parent.memory[y]
        else:
            return self.parent.memory[pag_loc][x] + self.parent.memory[pag_loc][y]
    
    def OP_ADC(self,adr, x,y,pag_loc=None):
        if not pag_loc:
            self.parent.memory[adr] = self.parent.memory[x] + self.parent.memory[y]
        else:
            self.parent.memory[pag_loc][adr] = self.parent.memory[pag_loc][x] + self.parent.memory[pag_loc][y]
            
                
    def OP_SUB(self,x,y,pag_loc=None):
        if not pag_loc:
            return self.parent.memory[x] - self.parent.memory[y]
        else:
    ...
    def OP_OR(self,x,y,pag_loc=None):
        if not pag_loc:
            return bool(self.parent.memory[x]) or bool(self.parent.memory[y]) 
        else:
            return bool(self.parent.memory[pag_loc][x]) or bool(self.parent.memory[pag_loc][y]) 
    
    def OP_AND(self,x,y,pag_loc=None):
        if not pag_loc:
            return bool(self.parent.memory[x]) and bool(self.parent.memory[y])
        else:
            
            return bool(self.parent.memory[pag_loc][x]) and bool(self.parent.memory[pag_loc][y])
    
    def OP_XOR(self,x,y,pag_loc=None):
        if not pag_loc:
            return bool(self.parent.memory[x]) != bool(self.parent.memory[y])
        else:
            return bool(self.parent.memory[pag_loc][x]) != bool(self.parent.memory[pag_loc][y])
    
    def OP_XNOR(self,x,y,pag_loc=None):
        if not pag_loc:
            return bool(self.parent.memory[x])==bool(self.parent.memory[y])
        else:
            return bool(self.parent.memory[pag_loc][x])==bool(self.parent.memory[pag_loc][y])
...
class CPU:
    def __init__(self, cores=None,memory=None,pagsize=None):
        ...
        if isinstance(memory, int) and pagsize:
            ...
            self.memory= [[None for x in range(pagsize)] for i in range(int(memory/pagsize))]
...
core1 = Core(queues=[Queue(quantum=1
                          ,processing_method ="rr"
                          )])

cpu = CPU(cores=[core1],memory=1024,pagsize=128)
...
instructions3 = [[1,2, 0],[1, 1, 1]]
for i in range(300):
    instructions3.append([1, 3+i,2, 2+i, 1+i])
#fibonacci



processes = [Process(instructions = instructions1,pag_loc=1)
             ,Process(instructions = instructions2,pag_loc=2),
             Process(instructions = instructions3,pag_loc=3)
             ]
